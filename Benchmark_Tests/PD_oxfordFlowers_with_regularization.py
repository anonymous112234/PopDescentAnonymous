...
from NN_models_oxfordFlowers import new_pd_NN_individual_without_regularization
...
def NN_optimizer_manual_loss(NN_object, batches, batch_size, epochs):
	...
	for images, labels in train_ds.take(26):
		random_batch_train_images, random_batch_train_labels = images, labels

	for images, labels in val_ds.take(5):
		random_batch_validation_images, random_batch_validation_labels = images, labels
...
def gradient_steps(lossfn, training_set, labels, batch_size, epochs, NN_object):
	...
	with tf.device('/device:GPU:0'):
		for e in range(epochs):
			...
			# grads = tape.gradient(model_loss, NN_object.nn.trainable_variables) ## with LR randomization and regularization loss
...
def evaluator(NN_object):
	...
	for images, labels in test_ds.take(5):
		random_batch_test_images, random_batch_test_labels = images, labels
...
def create_Parameters_NN_object(pop_size, randomization, CV_selection, rr):
	...
	object = individual_to_params(pop_size, new_pd_NN_individual_without_regularization, NN_randomizer_manual_loss, NN_optimizer_manual_loss, observer, randomization=randomization, CV_selection=CV_selection, rr=rr, history=history)
...
train_ds = train_ds.map(extract_images_labels)
val_ds = val_ds.map(extract_images_labels)
test_ds = test_ds.map(extract_images_labels)
...
if __name__ == "__main__":
	...
	for i in range(len(SEED)):
		...
		with open('../pd_CIFAR_benchmark_with_regularization.csv', 'a', newline = '') as file:
